...
<!--  v-if="options.length !== 0"-->
  <div class="dropdown">
    ...
    <label :for="'filter-switch_' + name" class="dropdown__options-filter">
      ...
      <ul class="dropdown__filter" :class="compact === true ? 'dropdown__filter-compact':''" role="listbox" tabindex="-1" >
        <li class="dropdown__filter-selected" ref="selected" aria-selected="true">
<!--          <nuxt-img loading="lazy" :src="'/images/affiliate/icon_' + selected_item.name + '.svg'" />-->
<!--          <nuxt-img loading="lazy" alt="payment icon" :src="selected_item.img_url"/>-->
          <div class="card-numbers">
            <span>{{ selected_item.name }}</span>
          </div>
        </li>
        <li>
          <ul class="dropdown__select" :class="compact === true ? 'compact-ul':''">
            <li
                ...
                :value="item"
                role="option"
                :class="compact === true ? 'compact-li':''"
                @click="selectCard(item)"
            >
<!--              <nuxt-img loading="lazy" :src="'/images/affiliate/icon_' + item.name.toString().toLowerCase() + '.svg'"/>-->
              <nuxt-img alt="payment icon" loading="lazy" :src="item.img_url"/>
              <div class="card-numbers">
                <span>{{ item.name }}</span>
              </div>
            </li>
...
.dropdown__filter-compact{
  border-radius: 15px;
  height: 55px;
  padding-left: 20px;
  border: 2px solid #D2D2D2 !important;
}
.dropdown__filter-compact li div{
  margin-top: 2px;
  font-size: 15px;
  font-weight: 600;
}
...
export default {
  ...
  methods: {
    ...
    selectCard: function (card) {
      this.selected_item = card;
      this.$emit("selected", card.id);
    },
  },
};
</script>
