<template>
  ...
  <div class="minibox__container">
    <p>{{$t("buybox.auth.new_password")}}</p>
    ...
    <div v-if="!password_changed">
      <div class="input-container" >
        ...
        <input type="password"  :placeholder="$t('buybox.auth.password_placeholder')" v-model="password"/>
      </div>
      <br>
      <div class="input-container">
        <nuxt-img loading="lazy" class="icon" src="/svg/user_pass.svg" alt="" />
        <input type="password"  :placeholder="$t('buybox.auth.password_confirm_placeholder')" v-model="password_confirm"/>
      </div>
      <br>
      <div class="btnbox">
        <a class="btn btn-primary" @click="doReset" style="background-color: #201D56;font-size: 14px;">
          ...
          <span v-else>{{$t('buybox.auth.password_reset_btn')}}</span>
        </a>
      </div>
    </div>
  </div>
...
export default {
  ...
  async setup() {
    ...
    const doReset = async () => {
      ...
      msg.value = await resetPassword(email, password.value, token, loading)
